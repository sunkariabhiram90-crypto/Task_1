...
function filterProducts(){
    ...
    let filtered = products.filter(product=>{
        let searchMatch = product.title.toLowerCase().includes(searchText);
...
function showStatistics(data){
    ...
    let total =data.reduce((sum,p)=>sum+p.price,0);
    document.getElementById("avgPrice")
    .innerText ="$"+(total/data.length).toFixed(2);
    let highest =[...data].sort((a,b)=>b.price-a.price)[0];
    let lowest =[...data].sort((a,b)=>a.price-b.price)[0];
    document.getElementById("highestPrice")
    .innerText ="$"+highest.price;
    document.getElementById("lowestPrice")
    .innerText ="$"+lowest.price;
}
function filterProducts(){
    let searchText =searchInput.value.toLowerCase();
    let category =categoryFilter.value;
...
function showStatistics(data){
    ...
    let totalPrice = data.reduce((sum,item)=>sum + item.price,0);
    let avgPrice =(totalPrice / data.length).toFixed(2);
    document.getElementById("avgPrice")
    .innerText = "$" + avgPrice;
    let highestProduct =[...data].sort((a,b)=>b.price-a.price)[0];
    let lowestProduct =[...data].sort((a,b)=>a.price-b.price)[0];
    document.getElementById("highestPrice")
    .innerText = "$" + highestProduct.price;
    document.getElementById("lowestPrice")
    .innerText = "$" + lowestProduct.price;
